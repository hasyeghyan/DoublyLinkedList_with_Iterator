...
template<typename T>
typename DLL<T>::Node* DLL<T>::merge(Node* l1, Node* l2) {
    Node dummy; 
    Node* tail = &dummy;
    
    while (l1 && l2) {
        if (std::less<T>{}(l1->val,l2->val)) {
            tail->next = l1;
            l1->prev = tail;
            l1 = l1->next;
        } else {
...
template<typename T>
typename DLL<T>::Node* DLL<T>::middleNode(Node* head) {
    Node* fast = head;
...
template<typename T>
typename DLL<T>::Node* DLL<T>::rec_sort(Node* head) {
